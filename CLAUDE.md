...
### ドキュメント更新の順序ルール
**重要**: ドキュメントに「完了」と記載する前に、必ず以下の順序を守ること

1. **実装**: コードを書く
2. **テスト実行**: 実際にスクリプトを実行する
3. **動作確認**: 正しく動作することを確認する
4. **ドキュメント更新**: 動作確認が完了してから初めてドキュメントを更新

**禁止事項**:
- ❌ テスト実行前にドキュメントに「完了」「✅」と書く
- ❌ 動作確認前にPhaseステータスを「完了」に変更
- ❌ 実装しただけでドキュメントを更新する

**例**:
```
✅ 良い: 実装 → テスト実行 → 動作確認 → ドキュメント更新
❌ 悪い: 実装 → ドキュメント更新（テストしていない）
```
...
#### アプローチ2: システムChrome + Persistent Profile（初回手動）
...
- **メリット**:
  - Spotify Web Playback SDKが完全動作
  - ログイン・Spotify連携が永続化（手動操作1回のみ）
  - 実際のユーザー環境に近い
- **デメリット**:
  - 初回セットアップに手動操作が必要
  - システムChromeのバージョンに依存
  - CI/CDへの組み込みが困難
- **推奨用途**: 本番運用、長期間のストリーミング

#### 使い分けガイドライン
- **開発・テスト**: アプローチ1（完全自動）
- **本番運用**: アプローチ2（システムChrome）を初回セットアップ後に使用
- **Spotify再生エラーが発生**: アプローチ2に切り替え
